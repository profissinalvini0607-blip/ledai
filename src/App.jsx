...
export default function App(){
  const [page,setPage]=useState(()=>{try{const s=localStorage.getItem("led_page");return s||PG.PERFIL;}catch{return PG.PERFIL;}});
  const [user,setUser]=useState(()=>{try{const s=localStorage.getItem("led_user");return s?JSON.parse(s):null;}catch{return null;}});
  const [isAdm,setIsAdm]=useState(()=>{try{return localStorage.getItem("led_isAdm")==="true";}catch{return false;}});
  ...
  // Persist session
  useEffect(()=>{try{localStorage.setItem("led_page",page);}catch{}},[page]);
  useEffect(()=>{try{if(user)localStorage.setItem("led_user",JSON.stringify(user));else localStorage.removeItem("led_user");}catch{}},[user]);
  useEffect(()=>{try{localStorage.setItem("led_isAdm",isAdm?"true":"false");}catch{}},[isAdm]);
  ...
  const doLogout=()=>{setUser(null);setIsAdm(false);setTurmas([]);setRels([]);setOcos([]);setVists([]);setChat([]);setMm(null);setSugs([]);setSelT(null);setHrs({});setPage(PG.PERFIL);setAdmView(null);setLoginMode("login");try{localStorage.removeItem("led_user");localStorage.removeItem("led_isAdm");localStorage.setItem("led_page",PG.PERFIL);}catch{}};
            ...
            <div key={a.email} style={{...S.rc,marginBottom:10}}>
              <div onClick={()=>setExpandedAdm(isExp?null:a.email)} style={{display:"flex",justifyContent:"space-between",alignItems:"center",cursor:"pointer"}}>
                <div style={{display:"flex",gap:8,alignItems:"center"}}>
                  ...
                  <span style={{color:C.mut,fontSize:12}}>({a.email})</span>
                  {a.blocked&&<span style={S.bdg("#dc2626")}>Bloqueado</span>}
                </div>
              ...
              {isExp&&<div style={{marginTop:14,paddingTop:14,borderTop:`1px solid ${C.brd}`}}>
                ...
                <div style={{marginBottom:12}}>
                  <label style={S.lbl}>Alterar senha</label>
                  <div style={{display:"flex",gap:8}}>
                    <input style={S.inp()} value={editAdmPw} onChange={e=>setEditAdmPw(e.target.value)} placeholder="Nova senha" type="password"/>
                    <button onClick={()=>{if(editAdmPw.length>=6){admUpdateSub(a.email,{senha:editAdmPw});setEditAdmPw("");}}} style={S.btn("secondary",true)}>Salvar</button>
                  </div>
                </div>
